...
export const AdminDashboard: React.FC = () => {
  ...
  const formatPhoneNumberMX = (phone: string | number): string => {
    // 1. Convertir a string y eliminar todo lo que no sea un número
    const cleaned = ('' + phone).replace(/\D/g, '');
    // 2. Verificar que tenga la longitud correcta de México (10 dígitos)
    const match = cleaned.match(/^(\d{2,3})(\d{3,4})(\d{4})$/);
    if (match) {
      // Si empieza con LADA de 2 dígitos (CDMX: 55, GDL: 33, MTY: 81)
      if (match[1].length === 2) {
        return `(${match[1]}) ${match[2]} - ${match[3]}`; // Resultado: (55) 1234-5678
      }
      // Para el resto del país con LADA de 3 dígitos
      return `${match[1]} - ${match[2]} - ${match[3]}`; // Resultado: 442-123-4567
    }
    // Si no tiene 10 dígitos, devuelve el texto limpio o original
    return cleaned;
  };
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-center px-6 py-2">
                <div className="px-6 pb-1 pt-2 bg-(--MediumBlue) rounded-xl items-center">
                  <button type="button" className="cursor-pointer" onClick={ location.pathname === '/admin/dashboard' ? (() => navigate('/admin/dashboard')) : ( ()=>{} ) }>
                    <svg
                      width="13"
                      height="15"
                      viewBox="0 0 12 13"
          ...
          <section className="stats-card-header">
            <div className="flex justify-between items-center">
              ...
              <div className="flex ">
                <div className="rounded-full px-6 pb-1 pt-2.5 bg-(--MediumBlue)">
                  <button type="button" className="cursor-pointer " onClick={() => navigate('/admin/historial')}>
                    <svg
                      width="17"
                      height="14"
                      viewBox="0 0 17 14"
                ...
                <div className="rounded-full px-4 pb-1 pt-2.5 ml-3 bg-(--MediumBlue)">
                  <button type="button" onClick={()=> setShowModalTargetahabiente(true)} className="cursor-pointer ">
                    <svg
                      width="27"
                      height="12"
                      viewBox="0 0 27 12"
            ...
            <div className="flex justify-between items-center py-2">
              <div className="bg-(--MediumBlue) rounded-full px-5 py-3 font-semibold text-[14px]">
                <span className="input-condensed">
                  No. de tarjetas <samp className="pr-5"></samp>{" "}
                  <strong className="text-white input-condensed">
                    {totalRegistros}
                  </strong>
                </span>
              </div>
              ...
              <div className="bg-(--MediumBlue) rounded-full text-right flex items-center justify-between px-5">
                <p className="text-[15px] font-bold tracking-wider text-slate-400 pr-14 py-2">
                  Cargos Globales
                </p>
                <p className="input-condensed text-2xl font-bold text-white tracking-tight py-1">
                  {formatearAPesos(montoTotalCargos)}
                </p>
              </div>
            </div>
          </section>
                      ...
                      <td className="rounded-tl-full rounded-bl-full rounded-tr-full rounded-br-full ">
                        ...
                        { item.estatus !== true ? 
                          ...
                          <div className="flex justify-center gap-2 ">
                            <button type="button" onClick={()=> setShowModalAviso(true)} className="action-icon-btn" title="Agregar gasto">
                              <span>
                        ...
                        :
                          <div className="flex justify-center gap-2 ">
                            ...
                            {/* Abono */}
                            <button type="button" onClick={() => setShowModalAbono(true)} className="action-icon-btn" title="Abono">
                              <span>
                                <svg
                                  width="16"
                                  height="10"
                            ...
                            {/* Historial */}
                            <button type="button" className="action-icon-btn" title="Historial">
                              <span>
                                <svg
                                  width="15"
                                  height="11"
                              ...
                              {grupo.items.map((compra: any, cIdx: any) => (
                                ...
                                >
                                  <div className="flex items-center">
                                    <span
                                      className={` font-normal tracking-wide transition-colors px-2 py-1 ${compra.precio.charAt(0) === "+" ? "text-(--VerdeNeon)" : "text-slate-200 group-hover:text-white"}`}
                                    >
                                      {compra.descr}
                                    </span>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-2.5">
                                    <span
                                      className={`input-condensed font-normal text-right px-2 py-1 transition-all ${compra.precio.charAt(0) === "+" ? "text-(--VerdeNeon)" : "text-slate-200 group-hover:text-white"}`}
                                    >
                                      {compra.precio}
                                    </span>
                                    <button
...
{/* BUENA PRACTICA ES MANEJAR LA VIDA DEL MODULO CONDICIONANDOLO PORQUE ASI DESTRUIMOS EL COMPONENTE POR COMPLETO */}
      ...
      />
        ...
        { showEditaModalAbono && ( 
          <ModalAbono
          // revisar con cary que al momento de actualizar la fecha en un abono se crea uno nuevo no se actualiza sobre el existente y al actualizar solo el monto
          // se crea un nuevo cargo con la nueva cantidad sobre la misma fecha 
          icono={
            <svg width="33" height="21" viewBox="0 0 33 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 19.2017V0.964355C0 0.671387 0.0895182 0.439453 0.268555 0.268555C0.447591 0.0895182 0.683594 0 0.976562 0H31.2622C31.5552 0 31.7871 0.0895182 31.958 0.268555C32.137 0.439453 32.2266 0.671387 32.2266 0.964355V19.2017C32.2266 19.4946 32.137 19.7306 31.958 19.9097C31.7871 20.0887 31.5552 20.1782 31.2622 20.1782H0.976562C0.683594 20.1782 0.447591 20.0887 0.268555 19.9097C0.0895182 19.7306 0 19.4946 0 19.2017ZM2.23389 17.1997C2.23389 17.6961 2.47803 17.9443 2.96631 17.9443H29.2603C29.7485 17.9443 29.9927 17.6961 29.9927 17.1997V2.97852C29.9927 2.4821 29.7485 2.23389 29.2603 2.23389H2.96631C2.47803 2.23389 2.23389 2.4821 2.23389 2.97852V17.1997ZM3.50342 16.3818V3.79639C3.50342 3.60107 3.60107 3.50342 3.79639 3.50342H13.3179C12.6831 4.15446 12.1867 5.04557 11.8286 6.17676C11.4705 7.2998 11.2915 8.59782 11.2915 10.0708C11.2915 11.5438 11.4705 12.8499 11.8286 13.9893C12.1948 15.1204 12.6994 16.0156 13.3423 16.6748H3.79639C3.60107 16.6748 3.50342 16.5771 3.50342 16.3818ZM12.7563 10.0708C12.7563 8.80127 12.8906 7.69043 13.1592 6.73828C13.4359 5.78613 13.8224 5.04557 14.3188 4.5166C14.8234 3.98763 15.4053 3.72314 16.0645 3.72314C16.748 3.72314 17.3462 3.98763 17.8589 4.5166C18.3797 5.04557 18.7826 5.78613 19.0674 6.73828C19.3522 7.69043 19.4946 8.80127 19.4946 10.0708C19.4946 11.3403 19.3522 12.4512 19.0674 13.4033C18.7826 14.3555 18.3797 15.1001 17.8589 15.6372C17.3462 16.1662 16.748 16.4307 16.0645 16.4307C15.4053 16.4307 14.8234 16.1662 14.3188 15.6372C13.8224 15.1001 13.4359 14.3555 13.1592 13.4033C12.8906 12.4512 12.7563 11.3403 12.7563 10.0708ZM18.8599 16.6748C19.5109 16.0156 20.0195 15.1204 20.3857 13.9893C20.7601 12.8499 20.9473 11.5438 20.9473 10.0708C20.9473 8.59782 20.7642 7.2998 20.3979 6.17676C20.0317 5.04557 19.5231 4.15446 18.8721 3.50342H28.4302C28.6255 3.50342 28.7231 3.60107 28.7231 3.79639V16.3818C28.7231 16.5771 28.6255 16.6748 28.4302 16.6748H18.8599Z" fill="#02FFA2"/>
            </svg>
